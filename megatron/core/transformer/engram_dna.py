...
"""Engram-DNA: a lightweight n-gram retrieval residual for Megatron-LM.
...
class EngramDNA(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.window_size < 1:
            raise ValueError(f"engram_dna_window_size must be >=1, got {self.window_size}")

        if len(self.dna_token_ids) != 5:
            raise ValueError(
                f"EngramDNA expects exactly 5 dna_token_ids (A,C,G,T,N order arbitrary), got {self.dna_token_ids}."
            )
        ...
        if self.window_size < max_n:
            raise ValueError(
                f"engram_dna_window_size ({self.window_size}) must be >= max(ngram_sizes) ({max_n})"
            )
        base_pows = [1]
    ...
    def _pool_windowed_ngram_embeddings(
        ...
        if L <= 0:
            raise ValueError(
                f"engram_dna_window_size ({self.window_size}) must be >= n ({n}) so that L=W-n+1 >= 1"
            )
        if L == 1:
            return e_end
    ...
    def forward(self, hidden_states: torch.Tensor, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        if input_ids is None:
            raise ValueError("EngramDNA requires input_ids, got None")
        ...
        for emb, n in zip(self.embeddings, self.ngram_sizes):
            ...
            if L_end < 1:
                ...
                raise ValueError(f"engram_dna_window_size ({W}) must be >= n ({n})")
