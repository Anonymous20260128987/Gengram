...
class TransformerLayer(GraphableMegatronModule, BaseTransformerLayer):
    ...
    def __init__(
        ...
        # [Optional: Engram-DNA n-gram retrieval residual]
        # IMPORTANT: For Megatron's layer-stacked dist checkpointing, every layer must have
        # the *same* parameter structure. Therefore we instantiate EngramDNA on *all* layers
        # when enabled, but only *activate* it on the configured layer ids.
        self.engram_dna = None
        self.engram_dna_active = False
        if getattr(self.config, 'engram_dna_enabled', False):
            from megatron.core.transformer.engram_dna import EngramDNA

            layer_ids = getattr(self.config, 'engram_dna_layer_ids', None)
            if layer_ids is None or len(layer_ids) == 0:
                active = True
            else:
                active = self.layer_number in layer_ids

            self.engram_dna_active = bool(active)
            self.engram_dna = EngramDNA(
                hidden_size=self.config.hidden_size,
                dna_token_ids=self.config.engram_dna_token_ids,
                ngram_sizes=self.config.engram_dna_ngram_sizes,
                embed_dim_per_ngram=self.config.engram_dna_embed_dim_per_ngram,
                active=active,
                use_short_conv=self.config.engram_dna_use_short_conv,
                short_conv_kernel_size=self.config.engram_dna_short_conv_kernel_size,
                short_conv_dilation=self.config.engram_dna_short_conv_dilation,
                window_size=self.config.engram_dna_window_size,
            )
    ...
    def _forward_attention(
        ...
        *,
        ...
        # Engram-DNA residual (active layers only)
        if self.engram_dna is not None and self.engram_dna_active:
            if input_ids is None:
                raise ValueError(
                    'Engram-DNA is enabled but input_ids was not provided. '
                    'Pass input_ids through TransformerBlock/GPTModel.'
                )
            hidden_states = hidden_states + self.engram_dna(hidden_states, input_ids)
