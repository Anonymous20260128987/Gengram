...
def add_megatron_arguments(parser: argparse.ArgumentParser):
    ...
    parser = _add_engram_dna_args(parser)
...
def _add_engram_dna_args(parser):
    group = parser.add_argument_group(title="engram-dna")

    group.add_argument("--engram-dna-enabled", action="store_true",
                       help="Enable Engram-DNA n-gram retrieval residual.")
    group.add_argument("--engram-dna-layer-ids", type=int, nargs="+", default=None,
                       help="Layer ids (1-based) to enable Engram-DNA, e.g. 1 15.")
    group.add_argument("--engram-dna-ngram-sizes", type=int, nargs="+", default=[2, 3],
                       help="N-gram sizes to use, e.g. 2 3.")
    group.add_argument("--engram-dna-window-size", type=int, default=1024,
                       help="Window size W (tokens) for Engram-DNA windowed pooling. Must be >= max(ngram_sizes).")
    group.add_argument("--engram-dna-embed-dim-per-ngram", type=int, default=128,
                       help="Embedding dim per n-gram table (concat then projected to hidden).")
    group.add_argument("--engram-dna-token-ids", type=int, nargs=5, default=[8, 5, 6, 7, 9],
                       help="Exactly 5 token ids for [A,C,G,T,N] (or your chosen order).")
    group.add_argument("--engram-dna-use-short-conv", action="store_true",
                       help="Enable depthwise short conv over retrieved values.")
    group.add_argument("--engram-dna-short-conv-kernel-size", type=int, default=4,
                       help="Kernel size for short conv.")
    group.add_argument("--engram-dna-short-conv-dilation", type=int, default=None,
